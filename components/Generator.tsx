...
const Generator: React.FC<GeneratorProps> = ({
  ...
  const [estimatedCost, setEstimatedCost] = useState<CostBreakdown | null>(
    null
  );
  ...
  useEffect(() => {
    if (activeScenario) {
      ...
      const initialCost = calculateScenarioCost(
        activeScenario.id,
        initialValues
      );
  ...
  const handleSaveToCart = () => {
    if (activeScenario && result) {
      const diagram = processDiagramTemplate(
        activeScenario.diagramCode,
        activeScenario.inputs,
        inputValues,
        globalVars
      );
  ...
  const getPrerequisites = () => {
    ...
    return activeScenario.prerequisites
        ...
        <div>
          <h2 className="text-xl font-bold text-white">
            {activeScenario
              ...
              : selectedCategory || 'Configuration Library'}
          </h2>
          <p className="text-sm text-slate-500">
            {activeScenario
              ? 'Configure deployment parameters'
              : 'Select a best-practice scenario'}
          </p>
        </div>
        {activeScenario && (
          <button
        ...
        {!activeScenario ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredScenarios.map((scenario) => (
                ...
                >
                  ...
                  <h3 className="text-lg font-semibold text-slate-200 mb-2 group-hover:text-white">
                    {scenario.title}
                  </h3>
              ...
              <button
                onClick={() =>
                  onScenarioChange({
                    ...
                    description:
                      'Describe complex requirements for the AI to generate.',
                    inputs: [],
                    learnLinks: [],
                    whatItDoes: [],
                    limitations: [],
              ...
              >
                ...
                <p className="text-sm text-slate-400 flex-1">
                  Generate bespoke scripts using Local Ollama.
                </p>
              </button>
            </div>
          </div>
          ...
          <div className="max-w-6xl mx-auto h-full">
            ...
            step === 'wizard' ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      <div className="p-3 bg-blue-900/20 border border-blue-900/30 rounded-lg text-xs text-blue-300 mb-4">
                        ...
                        {projectName && (
                          <div className="mt-1 pt-1 border-t border-blue-800/50 text-emerald-400">
                ...
                <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-emerald-950/10 border border-emerald-900/30 rounded-lg">
                            <div className="text-xs text-slate-400 max-w-xs">
                              <p className="italic">
                                * Estimates exclude bandwidth, IOPS, and taxes.
                  ...
                  {getPrerequisites().length > 0 && (
                    <div className="bg-blue-950/20 border border-blue-900/30 rounded-xl p-4">
                      ...
                      <p className="text-xs text-slate-400 mb-3">
                        For a successful deployment, ensuring the following
                        components exist is recommended:
                      </p>
                      <div className="flex flex-wrap gap-3">
                        {getPrerequisites().map(
                          (pre, idx) =>
                            pre && (
                              <button
                                key={idx}
                  ...
                  {activeScenario.commonIssues &&
                    activeScenario.commonIssues.length > 0 && (
                    ...
                    <div className="bg-emerald-950/20 border border-emerald-900/30 rounded-xl p-4 flex items-start gap-3">
                      ...
                      <div>
                        <h5 className="text-sm font-medium text-emerald-400">
                          Azure Context Active
                        </h5>
                        <p className="text-xs text-slate-400 mt-1">
                          Script will be generated for Subscription:{' '}
                    ...
                    <div className="bg-purple-950/20 border border-purple-900/30 rounded-xl p-4 flex items-start gap-3">
                      ...
                      <div>
                        <h5 className="text-sm font-medium text-purple-400">
                          Proximity Placement Group Active
                        </h5>
                        <p className="text-xs text-slate-400 mt-1">
                          Compute resources will be pinned to PPG:{' '}
                          <span className="font-mono text-slate-300">
                            {globalVars.proximityPlacementGroup}
                          </span>
                        </p>
            ...
            result ? (
              <ScriptDisplay
                result={result}
                diagramCode={processDiagramTemplate(
                  activeScenario.diagramCode,
                  activeScenario.inputs,
                  inputValues,
                  globalVars
