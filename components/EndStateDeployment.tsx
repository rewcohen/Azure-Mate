...
const EndStateDeployment: React.FC<EndStateDeploymentProps> = ({
  project,
  onRemoveItem,
}) => {
  ...
  const aggregatedCosts = project.items.reduce(
    (acc, item) => {
  ...
  const validationReport = useMemo(() => {
    ...
    project.items.forEach((item) => {
      ...
      if (scenarioDef?.prerequisites) {
        scenarioDef.prerequisites.forEach((reqId) => {
          if (!cartScenarioIds.has(reqId)) {
            ...
            errors.push(
              `'${item.scenarioTitle}' depends on '${reqTitle}', which is missing from the plan.`
            );
          }
        });
      }
      ...
      if (item.variables['location'])
        locations.add(String(item.variables['location']));
      if (item.variables['environment'])
        environments.add(String(item.variables['environment']));
    });

    // 3. Consistency Warnings
    if (locations.size > 1) {
      warnings.push(
        `Multi-Region Deployment detected: ${Array.from(locations).join(', ')}. Ensure you have planned for VNet Peering and data transfer costs.`
      );
    }
    if (environments.size > 1) {
      warnings.push(
        `Mixed Environments detected: ${Array.from(environments).join(', ')}. Typically, resources should belong to a single environment (e.g., all 'dev' or all 'prod').`
      );
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-500 bg-slate-950 p-8">
        ...
        <h2 className="text-xl font-bold text-slate-400 mb-2">
          Project Cart is Empty
        </h2>
        <p className="text-sm max-w-md text-center">
          Generate configurations in the library and click "Add to Project Cart"
          to build your end-state deployment plan.
        </p>
      </div>
    );
  }
      ...
      <div className="p-8 max-w-6xl mx-auto w-full space-y-8">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-medium px-2 py-1 rounded bg-slate-900 border border-slate-800 text-slate-400">
              {validationReport.passed
                ? 'Solution Valid'
                : `${validationReport.errors.length} Critical, ${validationReport.warnings.length} Warnings`}
            </span>
          </div>

          {validationReport.passed ? (
            <p className="text-sm text-slate-400">
              All configurations appear consistent. Dependencies are met, and
              regional settings match.
            </p>
          ) : (
            <div className="space-y-3">
              {validationReport.errors.map((err, idx) => (
                <div
                  key={`err-${idx}`}
                  className="flex items-start gap-3 p-3 rounded bg-red-900/20 border border-red-900/30"
                >
              ...
              {validationReport.warnings.map((warn, idx) => (
                <div
                  key={`warn-${idx}`}
                  className="flex items-start gap-3 p-3 rounded bg-amber-900/20 border border-amber-900/30"
                >
        ...
        <div className="space-y-4">
          ...
          {project.items.map((item) => (
            <div
              key={item.id}
              className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden transition-all hover:border-slate-600"
            >
              {/* Item Header */}
              <div
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-500 mt-0.5">
                      ...
                      <span>
                        {Object.keys(item.variables).length} Variables Tracked
                      </span>
                    </div>
                  </div>
                </div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {Object.entries(item.variables).map(([k, v]) => (
                              <div
                                key={k}
                                className="flex flex-col bg-slate-800/50 p-2 rounded border border-slate-800"
                              >
                                <span
                            ...
                            <tbody>
                              {item.costEstimate.items.map((cost, cIdx) => (
                                ...
                                >
                                  ...
                                  <td className="py-1.5 text-right font-mono text-emerald-500/80">
        ...
        <div className="flex justify-end pt-8">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full shadow-xl">
            ...
            <button
              ...
              onClick={() =>
                alert(
                  'This would export all scripts into a single ZIP or consolidated .ps1 file.'
                )
              }
