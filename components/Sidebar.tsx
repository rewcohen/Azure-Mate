...
import {
  fetchAzureStatus,
  sortServiceHealth,
} from '../services/azureStatusService';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleNavDoubleClick = (action: () => void) => {
    if (isWizardActive) {
      if (
        window.confirm('You have unsaved changes in the wizard. Leave page?')
      ) {
        onResetWizard();
        action();
      }
      ...
      <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-3">
              ...
              <div className="mb-2 pb-2 border-b border-slate-800">
                <p className="text-xs text-white font-medium truncate">
                  {azureContext.userDisplayName || 'User'}
                </p>
                <p className="text-[10px] text-slate-500 truncate">
                  {azureContext.username}
                </p>
              </div>
              ...
              <p
                ...
                title={azureContext.subscriptionId}
              >
        ...
        <div className="px-4 mb-2">
          ...
          <button
            ...
            onDoubleClick={() =>
              handleNavDoubleClick(() => onSelectView(ViewState.HOME))
            }
          ...
          <button
            onClick={() =>
              handleNavClick(() => onSelectView(ViewState.CATALOG))
            }
            onDoubleClick={() =>
              handleNavDoubleClick(() => onSelectView(ViewState.CATALOG))
            }
          ...
          <button
            onClick={() =>
              handleNavClick(() => onSelectView(ViewState.END_STATE))
            }
            onDoubleClick={() =>
              handleNavDoubleClick(() => onSelectView(ViewState.END_STATE))
            }
          ...
          <button
            onClick={() =>
              handleNavClick(() => onSelectView(ViewState.TROUBLESHOOTER))
            }
            onDoubleClick={() =>
              handleNavDoubleClick(() => onSelectView(ViewState.TROUBLESHOOTER))
            }
          ...
          <button
            onClick={() =>
              handleNavClick(() => onSelectView(ViewState.VARIABLES))
            }
            onDoubleClick={() =>
              handleNavDoubleClick(() => onSelectView(ViewState.VARIABLES))
            }
        ...
        <div className="bg-slate-900/50 rounded-lg p-3 border border-slate-800 hover:bg-slate-900 transition-colors cursor-default">
          ...
          {statusLoading ? (
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {criticalCount > 0
                  ? `${criticalCount} Critical Outages`
                  : warningCount > 0
                    ? `${warningCount} Services Warning`
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden w-80">
              ...
              <div className="max-h-80 overflow-y-auto p-0">
                {serviceStatuses.map((service, idx) => (
                  <div
                    key={idx}
                    className={`px-4 py-3 border-b border-slate-800 last:border-0 flex items-start gap-3 hover:bg-slate-800/50 transition-colors ${
