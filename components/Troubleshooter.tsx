...
const Troubleshooter: React.FC<TroubleshooterProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'console' | 'resources' | 'raw'>(
    'console'
  );
  ...
  const runAudit = () => {
    ...
    resources.forEach((r) => {
      ...
      if (r.type === 'Microsoft.Compute/virtualMachines') {
        r.properties.networkProfile?.networkInterfaces?.forEach(
          (nicRef: any) => {
      ...
      if (
        globalVars &&
        globalVars.location &&
        r.location.toLowerCase() !== globalVars.location.toLowerCase()
      ) {
        foundIssues.push({
          severity: 'info',
          resourceId: r.id,
  ...
  const handleLiveScan = async () => {
    ...
    try {
      addLog(
        `> Connect-AzAccount -Identity -TenantId "${azureContext.tenantId}"`
      );
      await new Promise((r) => setTimeout(r, 600));
      addLog(
        `> Authenticated as ${azureContext.userDisplayName} (${azureContext.username})`
      );
      addLog(
        `> Select-AzSubscription -SubscriptionId "${azureContext.subscriptionId}"`
      );

      await new Promise((r) => setTimeout(r, 800));
      addLog(
        `> Search-AzGraph -Query "Resources | project id, name, type, location, tags, properties" -First 1000`
      );
      addLog(`> Querying Azure Resource Graph... (Estimated time: 2s)`);

      // Fetch Mock Data
      const data = await mockSearchGraph(
      ...
      addLog(
        `> Checking compliance with '${globalVars.projectPrefix}' standards...`
      );
  ...
  const getDiagramCode = (issue: AuditIssue): string | null => {
    const resource = resources.find(
      (r) => r.id.toLowerCase() === issue.resourceId.toLowerCase()
    );
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl max-w-2xl w-full flex flex-col overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto">
              <div className="mb-4 space-y-2 text-sm text-slate-300">
                <p>
                  Use this method if you cannot connect the app directly to
                  Azure.
                </p>
              </div>
              <div className="relative group">
                <div className="absolute top-2 right-2">
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 bg-slate-700 hover:bg-slate-600 text-slate-200 text-xs rounded"
                  >
                    {copiedScript ? (
      ...
      <div className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-950">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">
              {azureContext.isConnected
                ...
                : 'Offline Infrastructure Auditor'}
            </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          {azureContext.isConnected && (
            ...
            >
              <Zap
                className={`w-4 h-4 ${scanStatus === 'scanning' ? 'animate-pulse' : 'fill-current'}`}
              />
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-950">
            {scanStatus === 'scanning' ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-500">
                ...
                <p>
                  No issues detected{' '}
                  {resources.length > 0 ? 'in current config' : ''}.
                </p>
                {!azureContext.isConnected && resources.length === 0 && (
                  <p className="text-xs mt-2">
                    Connect to Azure or Import JSON to start.
                  </p>
                )}
              </div>
            ) : (
        ...
        <div className="w-1/2 flex flex-col h-full bg-[#0d1117]">
          <div className="flex items-center border-b border-slate-800 bg-slate-900">
            ...
            >
              <List className="w-4 h-4" /> Detected Resources (
              {resources.length})
            </button>
            <button
            ...
            {viewMode === 'console' && (
              ...
              >
                <div className="text-slate-500 mb-4">
                  # Azure PowerShell / Resource Graph Output
                </div>
                {consoleLogs.length === 0 && !azureContext.isConnected && (
                  <div className="text-slate-600 italic">
                    Waiting for connection... Use "Import JSON" for offline
                    mode.
                  </div>
                )}
                {consoleLogs.map((log, i) => (
                  <div
                    key={i}
                    className={`${log.startsWith('>') ? 'text-emerald-400' : 'text-slate-300'} break-all`}
                  >
                    {log}
                  </div>
                ))}
              ...
              <div className="absolute inset-0 overflow-y-auto">
                <table className="w-full text-left text-xs border-collapse">
                  <thead className="bg-slate-900 text-slate-500 sticky top-0 z-10">
                    <tr>
                      <th className="p-3 font-medium border-b border-slate-800">
                        Name
                      </th>
                      <th className="p-3 font-medium border-b border-slate-800">
                        Type
                  ...
                  <tbody className="text-slate-300 divide-y divide-slate-800">
                    {resources.map((r, i) => (
                      <tr key={i} className="hover:bg-slate-800/30">
                        <td
                          className="p-3 font-medium text-blue-300 truncate max-w-[150px]"
                          title={r.name}
                        >
                          {r.name}
                        </td>
              ...
              <textarea
                ...
                placeholder={
                  azureContext.isConnected
                    ...
                    : 'Paste your azure_snapshot.json content here...'
                }
              />
            )}
          </div>
