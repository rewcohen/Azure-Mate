...
const PRICING_CATALOG: Record<string, number> = {
  ...
  'Bastion (Standard)': 211.7, // $0.29/hr
  'Front Door (Standard Base)': 35.0,
  'Front Door (Data Process 1TB)': 10.0, // Estimate
  'VPN Gateway (VpnGw1)': 138.7, // ~$0.19/hr
  'VPN Gateway (VpnGw2)': 262.8, // ~$0.36/hr
  ...
  'ACR (Premium)': 50.0, // Estimate daily rate x 30
  ...
  // Security
  'Key Vault (Premium Base)': 10.0, // Roughly estimated monthly base if actively used
  'Azure Firewall (Basic)': 288.0, // ~$0.395/hr
  'Azure Firewall (Standard)': 912.0, // ~$1.25/hr
  'Azure Firewall (Premium)': 1277.0, // ~$1.75/hr
  'App Gateway (WAF v2)': 320.0, // ~$0.44/hr fixed
  'Log Analytics (Per GB)': 2.3,
  'Sentinel (Per GB Analysis)': 2.0, // On top of LA

  // Serverless
  'Static Web App (Standard)': 9.0,
  'Container Apps (vCPU)': 25.0, // Approx active monthly
  'Container Apps (Memory)': 6.0, // Approx active monthly

  // ACI
  'ACI (vCPU)': 30.0, // Est monthly per core active
  'ACI (Memory)': 3.0, // Est monthly per GB active

  // Integration
  'Logic App (WS1)': 175.0, // Approx
  'Logic App (WS2)': 350.0,
  'Logic App (WS3)': 700.0,
  'APIM (Developer)': 48.0,
  'APIM (Standard)': 147.0,
  'APIM (Premium)': 2795.0,
  'Service Bus (Standard Base)': 10.0, // Base charge
  ...
  try {
    ...
    if (data.Items && Array.isArray(data.Items)) {
      data.Items.forEach((item: any) => {
        ...
        if (
          item.armSkuName &&
          PRICING_CATALOG.hasOwnProperty(item.armSkuName)
        ) {
...
export const calculateScenarioCost = (
  ...
  switch (scenarioId) {
    ...
    case 'static-web-app': {
      ...
      const price =
        sku === 'Standard' ? PRICING_CATALOG['Static Web App (Standard)'] : 0;
      items.push({
    ...
    case 'container-apps': {
      ...
      items.push({
        ...
        unitPrice:
          PRICING_CATALOG['Container Apps (vCPU)'] * 0.5 +
          PRICING_CATALOG['Container Apps (Memory)'],
        quantity: 1,
        total:
          PRICING_CATALOG['Container Apps (vCPU)'] * 0.5 +
          PRICING_CATALOG['Container Apps (Memory)'],
    ...
    case 'network-vpn-gateway': {
      ...
      const price = PRICING_CATALOG[key] || 138.7;
      items.push({
    ...
    case 'storage-datalake-gen2': {
      ...
      const unitPrice = isGrs
        ? PRICING_CATALOG['Storage Capacity (Hot GRS)']
        : PRICING_CATALOG['Storage Capacity (Hot LRS)'];
    ...
    case 'azure-firewall': {
      ...
      const price = PRICING_CATALOG[key] || 912.0;
    ...
    case 'logic-app-standard': {
      ...
      const price = PRICING_CATALOG[`Logic App (${sku})`] || 175.0;
      items.push({
    ...
    case 'apim-standard': {
      ...
      const price = PRICING_CATALOG[`APIM (${sku})`] || 48.0;
      items.push({
